...
if len(good) > MIN_MATCH_COUNT:
    ...
    # 从本质矩阵恢复相机位置和朝向
    _, R, t, _ = cv2.recoverPose(E, src_pts, dst_pts, K0)

    # 假设相机1的位置为原点 [0, 0, 0]
    cam_pos_1 = np.array([0, 0, 0])
    # 相机2的位置可以通过 R 和 t 计算出来
    cam_pos_2 = (-R.T @ t).ravel()
    ...
    # 计算视差图
    disparity = stereo.compute(cv2.cvtColor(img1_color, cv2.COLOR_BGR2GRAY), cv2.cvtColor(img2_color, cv2.COLOR_BGR2GRAY)).astype(np.float32) / 16.0

    # 为了将视差图转换为深度图，我们需要创建一个重投影矩阵Q
    # 假设焦距为 f, 基线距离为 b, 图像的中心坐标为 cx, cy
    f = calibration_data['cam0'][0, 0] # 焦距
    b = calibration_data['baseline'] # 基线
    cx = calibration_data['cam0'][0, 2] # 图像中心x坐标
    cy = calibration_data['cam0'][1, 2] # 图像中心y坐标
    ...
    # 获取对应于三维点的颜色信息
    colors = cv2.cvtColor(img1_color, cv2.COLOR_BGR2RGB)
    ...
    sample = np.random.choice(len(output_points), size=20000, replace=False)

    # 绘制散点图
    ax.scatter(
        output_points[sample, 0], output_points[sample, 1], output_points[sample, 2],
        c=output_colors[sample]/255, marker='.'
    )

     # 绘制相机位置
    ax.scatter(*cam_pos_1, c='r', marker='o', s=100, label='Camera 1')
    ax.scatter(*cam_pos_2, c='b', marker='^', s=100, label='Camera 2')

    # 绘制相机朝向
    cam_dir_1 = R[:, 0]  # 取R的第一列作为相机1的朝向
    cam_dir_2 = -R[:, 0]  # 相机2朝向与相机1相反
    ax.quiver(*cam_pos_1, *cam_dir_1, length=0.3, color='r')
    ax.quiver(*cam_pos_2, *cam_dir_2, length=0.3, color='b')
