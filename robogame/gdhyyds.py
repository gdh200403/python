...
def move(direction, speed):
    ...
    speed: 移动速度，'H' 表示高速，'M' 表示中速，'L' 表示低速
    """
...
def fire(shoot_distance):
    ...
    """
    is_need_shoot_faster = shoot_distance > 100  # 距离大于100cm时使用高速发射
...
def aim():
    ...
    '''
    LR_DISTANCE_THRESHOLD = 200  # 设定的距离阈值，单位px
    ...
    while not is_aimed:
        if abs(lr_distance) < LR_DISTANCE_THRESHOLD:
            ...
            move('A' if lr_distance < 0 else 'D', 'L')
...
def rush_and_catch(cam_id):
    ...
    DECELERATION_TIME = 2  # 减速运动时间，单位s
...
def is_block_involved():
    ...
    功能：判断车内是否有方块
    '''
    INVOLVED_DISTANCE_THRESHOLD = 100  # 设定的阈值，单位cm
