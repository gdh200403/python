...
LOCAL_0 = 150
SPEED_LR_C = 0.5 # 左右慢速移动速度，单位为cm/s
...
def rush_and_catch(cam_id):
    ...
    '''           
    DECELERATION_DISTANCE = 50  # 减速距离，单位px
...
def get_distance_inside():
    ...
    if num:
        try:
            data = str(binascii.b2a_hex(ser_1.read(num)))
            if(len(data) > 8):
                dat1 = data[6:7]
                dat2 = data[7:8]
                dat3 = data[8:9]
                ...
                distance_inside = ((((int(change(dat1)) * 16) + int(change(dat2))) * 256) + ((int(change(dat3)) * 16) + int(change(dat4))) / 10)
